...
In this vignette we illustrate how to compute short term trend with `csalert::short_term_trend` for multiple locations, and visualize in a map. We also include labels with the number of confirmed COVID cases per 100 000 population (for each county).
...
```{r}
d_msis <- cstidy::nor_covid19_cases_by_time_location_csfmt_rts_v1
...
On this specific subset, we carry out one function like in the previous example, `csalert::shorterm_trend`. Return the result at the end of the action function.
...
q <- q + csstyle::scale_fill_fhi("Trend",palette = "warning", direction = 1, drop=F)
